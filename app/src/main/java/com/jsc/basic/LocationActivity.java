...
public class LocationActivity extends ABaseActivity {
    ...
    public void onPermissionLaunchBack(String[] unGrantPermissions) {
        ...
        if (unGrantPermissions.length == 0) {
            List<String> providers = Locator.getInstance().getLocationManager().getAllProviders();
            StringBuilder builder = new StringBuilder();
            for (String p : providers) {
                builder.append(p).append("\u3000");
            }
            binding.tvTips.setText(builder.toString());
            Locator.getInstance().requestLocationUpdates(this,
                    5_000L,
                    5,
...
//                            binding.tvLocation.setText(location.toString());
                        ...
                        @Override
                        public void onSatelliteStatusChanged(@NonNull GnssStatus src) {
                            super.onSatelliteStatusChanged(src);
                            if (isStatistics) return;
                            isStatistics = true;
                            final GnssStatus status = src;
                            //总数量
